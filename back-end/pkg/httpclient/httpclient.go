...
var (
	sharedClient = &http.Client{Transport: otelhttp.NewTransport(
...
func NewClient(options ...func(*Client)) *Client {
	c := &Client{
		...
		tracer:                trace.NewNoopTracerProvider().Tracer("default httpclient"),
	}
...
// Put executes http PATCH request with the given body & headers.
// The supported data types for body are:
// - string
// - []byte
// - struct/map/slice : will be marshalled to JSON
// - io.Reader
func (c *Client) Put(ctx context.Context, url string, headers http.Header, body interface{}) (*http.Response, error) {
	return c.doMethod(ctx, http.MethodPut, url, body, headers)
}

// PutJSON executes http PATCH request with the given body & headers and parse the json response body
// into the given data.
// The supported data types for body are:
// - string
// - []byte
...
// - io.Reader
// The returned response Body is already closed
func (c *Client) PutJSON(ctx context.Context, url string, headers http.Header, body, data interface{}) (*http.Response, error) {
	return c.doMethodJSON(ctx, http.MethodPut, url, body, headers, data)
}
